...
func main() {
	...
	const taskName = "Fast Doubling" // Define task name as a constant

	log.Printf("Calculating F(%d) using %s with a timeout of %v...", n, taskName, timeout)
	...
	go func() {
		...
		progressPrinter(ctx, progressAggregatorCh, taskName) // Pass taskName directly
	}()
	...
	go func() { // Removed currentTask argument
		defer wg.Done()
		start := time.Now()
		v, err := fibFastDoubling(ctx, progressAggregatorCh, n, intPool) // Call fibFastDoubling directly
		duration := time.Since(start)
		resultsCh <- result{taskName, v, duration, err} // Use taskName for result
	}()
